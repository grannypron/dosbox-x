...
bool waitForString(std::string s, uint64_t start_address) {
    ...
    uint8_t value;
    int MAX_SLEEP_INTERATIONS = 10;
    int sleep_iterations = 0;
    while(sleep_iterations < MAX_SLEEP_INTERATIONS) {
        for(int idx = 0; idx < s.length() && sleep_iterations < MAX_SLEEP_INTERATIONS; idx++) {
            mem_readb_checked((PhysPt)start_address + idx, &value);
            if(value != s.at(idx)) {
                idx = 0;
                sleep_iterations++;
